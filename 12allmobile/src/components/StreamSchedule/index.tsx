...
const StreamSchedule: FC<Props> = ({
    ...
    if (epgEntries.length) {
      ...
      epgEntries.forEach((entry: EpgEntry) => {
        ...
        if (start >= now || now <= end) {
          ...
          const day = `${padWithZero(start.getDate())}.${padWithZero(
            start.getMonth() + 1
          )}`;
          ...
          scheduleObj[WEEKDAYS[start.getDay()]].shows.push(
            new Show(entry.id, startDisplay, endDisplay, entry.title, day)
          );
        }
      });
  ...
  useEffect(() => {
    if (search) {
      ...
      Object.keys(scheduleRef.current).forEach((key) => {
        const shows = scheduleRef.current[key].shows.filter(
          ({ title }) => title && title.toLowerCase().includes(searchToLower)
        );
        if (shows.length) {
  ...
  const onScroll = () => {
    if (daysRef.current?.length && !segmentChangingRef.current) {
      const element = daysRef.current?.find(
        (d) =>
          d.getBoundingClientRect().top >= 160 &&
          d.getBoundingClientRect().top <= 300
      );
  ...
  return (
    ...
    >
      ...
      {!loading ? (
        <>
          <AppSearchbar onSearchChange={setSearch} />
          <IonSegment className="schedule-days" value={selectedDay}>
            {Object.keys(schedule).map((key) => (
              <IonSegmentButton
                key={`${key}_${schedule[key].day}`}
                value={`${key}_${schedule[key].day}`}
                onClick={() =>
                  handleSegmentButtonClick(`${key}_${schedule[key].day}`)
                }
              >
          ...
          <IonCard
            className={`epg-entries-card ${
              Object.keys(schedule).length === 0 ? "ion-hide" : ""
            }`}
            ref={epgCardRef}
          >
            <IonCardContent>
              {Object.keys(schedule).map((key) => (
                <div
                  key={`${key}_${schedule[key].day}`}
                  id={`${key}_${schedule[key].day}`}
                  className="epg-entities-day-container"
                >
                  <IonText color="primary" className="day">
                    {t(`days.${key}`)} {schedule[key].day}
                  </IonText>

                  {schedule[key].shows.map(
                    ({ id, start, end, title }: Show) => (

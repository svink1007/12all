...
export default class VertoConferenceManager {
  ...
  constructor(params: VertoConferenceManagerParams) {
    ...
    this.vertoNotification.onWebSocketMessage.subscribe((message) => {
      if (message.params?.pvtData?.action === "conference-liveArray-join") {
        ...
        const { callID, chatChannel, infoChannel, modChannel } = message.params
        ...
        } else if (params.imHost && chatChannel) {
          this._moderatorChannel = chatChannel.replace("chat", "mod");
        }
      } else if (
        message.method === "verto.event" &&
        message.params?.eventChannel === this.chatChannel
      ) {
        this.handleChatEvent(message.params);
      } else if (
        message.method === "verto.event" &&
        message.params?.eventData?.canvasInfo
      ) {
        this.vertoNotification.onLayoutChange.notify(
          message.params.eventData.canvasInfo.layoutName
        );
      }
    });
  }
  ...
  private handleChatEvent({ data }: any) {
    try {
      const { message, method, to, from, fromDisplay }: OutgoingMessage =
        JSON.parse(data.message);

      switch (method) {
        case ChatMethod.ToEveryone:
          message &&
            this.vertoNotification.onChatMessageToAll.notify(
              new IncomingMessage(
                to,
                fromDisplay || data.fromDisplay,
                message,
                this._primaryId === from
              )
            );
          break;
        ...
        case ChatMethod.SwitchHostStream:
          if (this._primaryId === to) {
            ...
            this.vertoNotification.onChatMessageSwitchHostStream.notify(
              new SwitchHost(username, password, to)
            );
        ...
        case ChatMethod.ChangeParticipantState:
          if (message) {
            try {
              const m = JSON.parse(message);
              this.vertoNotification.onChatMessageChangeParticipantState.notify(
                m
              );
            } catch (e) {
        ...
        case ChatMethod.MakeCoHost:
          ...
          if (
            this._primaryId &&
            callIds.indexOf(this._primaryId) !== -1 &&
            message
          ) {
            try {
              const m = JSON.parse(message);
  ...
  private broadcast(
    ...
    this.vertoNotification.sendWebSocketRequest.notify({
  ...
  private broadcastModeratorCommand(
    ...
    console.log(
      "FIRST BROADCAST MODERATOR COMMAND:",
      command,
      memberId,
      argument,
      !this._moderatorChannel
    );
  ...
  runDebugCommand(command: string, argument?: string | any[]) {
    this.broadcastRoomCommand(command, argument);
  }
  ...
  moderateMemberById(memberId: string) {
    const constantBroadcasterFor =
      (command: string) => (argument?: string | any[]) => () => {
        this.broadcastModeratorCommand(command, memberId, argument);
      };

    const parameterizedBroadcasterFor =
      (command: string) => (argument?: string | any[]) => {
        this.broadcastModeratorCommand(command, memberId, argument);
      };

...
import { Action, StreamDebugActions, StreamDebugRedux } from "../types";
...
export default function reducer(
  state = INITIAL,
  { type, payload }: Action<StreamDebugActions>
): StreamDebugRedux {
  switch (type) {
    ...
    case STREAM_DEBUG_SET_REPLACE_SENT_STREAM:
      ...
      if (
        state.sentStream?.getAudioTracks().length &&
        payload.sentStream?.getVideoTracks().length
      ) {
        mediaStream = new MediaStream([
          state.sentStream.getAudioTracks()[0],
          payload.sentStream.getVideoTracks()[0],
        ]);
      } else if (
        state.sentStream?.getVideoTracks().length &&
        payload.sentStream?.getAudioTracks().length
      ) {
        mediaStream = new MediaStream([
          payload.sentStream.getAudioTracks()[0],
          state.sentStream.getVideoTracks()[0],
        ]);
      }

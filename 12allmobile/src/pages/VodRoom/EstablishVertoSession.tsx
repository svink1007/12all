...
type Props = {
  ...
  isVodRoom?: boolean; // New prop to explicitly identify VOD rooms
  caller: string;
  streamName: string;
      ...
      vs.notification.onBootstrappedParticipants.subscribe(
        (bootParticipants: Participant[]) => {
          ...
          if (me) {
            ...
            // For VOD rooms, we don't want to automatically toggle camera/mic
            // Only handle camera/mic for regular stream rooms
            if (isVodRoom) {
              // For VOD rooms, ensure camera and mic are muted by default
              if (!me.audio.muted) {
                vs.togglePrimaryMic();
              }
              if (!me.video.muted) {

...
import {SharedVodVlrs} from "../../../shared/types";
...
const WatchPartySession: FC<Props> = ({
                                        onProgress,
                                        vod, id,
                                        volume, loop, setIsPlaying, setDuration, setVodStatus,
                                        onShowProgressLeave,
                                        onShowLoading,
                                        onVertoSession,
                                        onParticipants,
  ...
  useEffect(() => {
    if (share === ShareStreamOption.Stream || share === ShareStreamOption.File  || share === ShareStreamOption.VoD) {
          ...
          if(data){
            ...
            try {
              ...
              if(json['playing']['Status'] !== 'Playing' && loop && vod.current?.url != null){
                vertoSessionRef.current?.sendDebugAction("vod_play", `${getVodName(vod.current.url)}`, "conf-control");
              }
            } catch (err: any) {
  ...
  }, [vertoSessionRef, loop, vod.current]);
  ...
  useEffect(() => {
    if(vertoSessionRef.current && vod && vertoSessionRef.current.getModeratorChannel() !== null){
      ...
      const vodName = getVodName(vod.current?.url!);
      vertoSessionRef.current?.sendDebugAction("vod_play", `${vodName}`, "conf-control");
    }
  }, [vertoSessionRef.current, vod, vertoSessionRef.current?.getModeratorChannel()]);

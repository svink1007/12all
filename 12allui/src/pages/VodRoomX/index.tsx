...
import {MapPublicId, RoomLayout, SharedStream, SharedVodVlrs, Vlr} from '../../shared/types';
...
import initVoD from "../VodRoomX/initVod";
...
import {addVlrLogo} from "../WatchParty/Start/Step2";
...
const VodRoom: FC<RouteComponentProps> = ({history}: RouteComponentProps) => {
    ...
    const startNewRoom = useCallback(() => {
        startStreamVlr({
            ...
            setImHost,
            onReady: () => {
                startVodRoom().catch(console.error);
            }
        });
    }, []);
    ...
    const handleRefChange = (node: any) => {
        if (node !== null) {
            // console.log('DOM node changed:', node);
        }
    };
        ...
        try {
            ...
            if (
                ...
                if (
                    ...
                    console.log("[TEST VERTO] Playing VOD:", vodName);
    ...
    const startVodRoom = useCallback(async (params?: RedirectToLivingRoom) => {
    ...
    // useEffect(() => {
    //     console.log("CALLER ID", vertoSessionRef.current)
    // }, [vertoSessionRef.current]);
    ...
    const togglePlay = async () => {
        const playing = !isPlaying; // Toggle value
        ...
        if (videoRef.current && sharedStreamData.current) {
            if(vodStatus === "Idle"){
                ...
                if (playing) {
                    ...
                    vertoSessionRef.current?.sendDebugAction("a_play_pause", getVodName(sharedStreamData.current.url), "conf-control")
                } else {
                    vertoSessionRef.current?.sendDebugAction("a_play_resume", getVodName(sharedStreamData.current.url), "conf-control")

...
type Props = {
  sharedStreamData?: SharedStreamVlrs;
  logo: string |null;
  streamName?: string;
  roomId?: string;
  sessionName?: string;
...
const TopBarStream: FC<Props> = ({streamName,logo, roomId, participants, onExit,sessionName,sharedStreamData}: Props) => {
    ...
    <IonToolbar className="top-bar-stream-share" >
      <div className="flex justify-between items-center"  >
        <div className="flex items-center ">
          ...
          style={{color: 'var(--ion-color-secondary-new-contrast)'}}>
            {roomId ? <>Room Id:{roomId}</>: <> Channel</>} 
          </IonLabel>
        ...
        <div>
          <div className={"flex justify-start items-center mr-5"}>
            <Header2 sharedStreamData={sharedStreamData}/>

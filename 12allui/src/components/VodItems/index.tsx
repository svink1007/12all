import React, {FC, useEffect, useState} from 'react';
import './styles.scss';
import {
    IonButton, IonButtons, IonCard, IonCardContent, IonCardHeader, IonCardSubtitle, IonCardTitle,
    IonCheckbox, IonIcon, IonImg,
    IonItem,
    IonList, IonPopover, IonSpinner,
} from '@ionic/react';
import {Routes} from "../../shared/routes";
import {
    ellipsisVerticalSharp,
    createOutline,
    trashOutline,
    informationCircleOutline, gridOutline, star, listOutline
} from "ionicons/icons";
import {AlertDialog, Menu} from '@base-ui-components/react';
import {VodChannelItem, VodState} from "../../redux/reducers/vodReducers";
import {VodService} from "../../services/VodService";
import {deleteVod, loadVods} from "../../redux/actions/vodActions";
import {setErrorToast, setSuccessToast} from "../../redux/actions/toastActions";
import {useDispatch} from "react-redux";

export function secondsToHMS(seconds: number): string {
    const h = Math.floor(seconds / 3600);
    const m = Math.floor((seconds % 3600) / 60);
    const s = seconds % 60;
    return h > 0
        ? `${h.toString().padStart(2, '0')}:${m.toString().padStart(2, '0')}:${s.toString().padStart(2, '0')}`
        : `${m.toString().padStart(2, '0')}:${s.toString().padStart(2, '0')}`;
}

export function formatDate(isoString: string): string {
    const date = new Date(isoString);
    const day = date.getUTCDate().toString().padStart(2, '0');
    const month = (date.getUTCMonth() + 1).toString().padStart(2, '0'); // Months are 0-based
    const year = date.getUTCFullYear();
    return `${day}.${month}.${year}`;
}

export interface Vod {
    id: string | number;
    title: string;
    logo?: string;
    isPrivate: boolean;
    duration: number;
    genre: string;
    description: string;
    created_at: string;
}

export interface VodListProps {
    vod: VodState;
    loading?: number | null;
    selection: string[];
    handleCheckboxChange: (value: string, checked: boolean) => void;
    handleLoading: (value: number | null) => void;
    openModal: (value: VodState) => void;
}

export interface VodChannelProps {
    channel: VodChannelItem;
    openModal: (value: VodChannelItem) => void;
}

export const placeholderImage = "data:image/png;base64,"


const FallbackImage = ({src, alt, placeholder}: { src: string; alt?: string; placeholder: string }) => {
    
    const [imgSrc, setImgSrc] = useState(src);
    useEffect(() => {
        setImgSrc(src);
    }, [src]);
    return (
        <IonImg
            className={`rounded-img !rounded-5 ${imgSrc === null ? "!h-[100px] w-auto" : "w-[100%] !h-auto"}`}
            src={imgSrc}
            alt={alt}
            style={{
                "--border-radius": "20px",
                borderRadius: "20px",
            }}
            onIonError={() => setImgSrc(placeholder)}
        />
    );
};


export const VodList: React.FC<VodListProps> = ({
                                                    vod,
                                                    loading, handleLoading,
                                                    selection, openModal,
                                                    handleCheckboxChange,
                                                }) => {
    const [profilePopover, setProfilePopover] = useState<{
        showPopover: boolean;
        event?: Event;
    }>({showPopover: false});

    const dispatch = useDispatch();

    const handleClick = (event: any) => {
        event.persist();
        setProfilePopover({showPopover: true, event});
    };

    const handleDelete = async (event: any) => {
        setProfilePopover({showPopover: false, event});
        setIsDialogOpen(true);
    };

    const deleteItem = async (event: any) => {
        handleLoading(vod.id);
        const response = await VodService.deleteVod(vod.id);
        if (response.data.status === "ok") {
            dispatch(deleteVod(vod.id))
            dispatch(setSuccessToast("VOD data deleted"));
        } else {
            dispatch(setErrorToast(response.data.message));
        }
        handleLoading(null);
    };

    const [isDialogOpen, setIsDialogOpen] = useState(false);
    return (
        <tr className={"!border-t-[#707070]"}
            style={{borderTopWidth: "1px"}}>
            <th scope="row">
                <IonCheckbox
                    checked={selection.includes(`${vod.id}`)}
                    onIonChange={(e) => handleCheckboxChange(`${vod.id}`, e.detail.checked)}/>
            </th>
            <td>
                <div className={'flex items-start justify-start gap-3'}>
                    <div
                        className={'bg-black !min-w-[133.33px] !w-[133.33px] h-[75px] flex items-center justify-center'}>
                        <img
                            alt={vod.title}
                            style={{objectFit: "contain"}}
                            className={`!rounded-md ${vod.logo ? 'w-[auto] h-[75px]' : 'w-[133.33px]'} h-[75px]`}
                            src={vod.logo && vod.logo.length > 0 ? vod.logo : placeholderImage}
                        />
                    </div>
                    <div className={'flex justify-start flex-col'}>
                        <span className={"mt-2 truncate !w-[250px]"}>{vod.title}</span>
                        {
                            vod.starsAmount && vod.starsAmount > 0 ?
                                <div className="text-[#D4D4D4] !text-[0.8rem] mt-3" style={{display: "flex", alignItems: "center"}}>
                                    <span style={{marginRight: "8px"}}>Cost:</span>
                                    <b className={'text-white'}>{vod.starsAmount}</b> &nbsp;
                                    <IonIcon icon={star} size={"md"} style={{color: "#e91e63"}}/>
                                </div> : <></>
                        }
                    </div>
                </div>
            </td>
            <td>
                <div className={'flex items-start justify-start !h-[100%]'}>
                    {vod.isPrivate || vod.url.trim().length<3? (
                        <span className="!text-[#5C7ECF] !mt-3.5">Private</span>
                    ) : vod.starsAmount && vod.starsAmount > 0 ? (
                        <span className="!text-[#E966AE] !mt-3.5">Paid</span>
                    ) : (
                        <span className="!text-[#31E36C] !mt-3.5">Public</span>
                    )}
                </div>
            </td>
             <td>
                <div className={'flex items-start justify-start !h-[100%]'}>
                    {vod.url.trim().length<3 ?  (
                        <span className="!mt-3.5">Not published</span>
                    ) : (
                        <span className="!mt-3.5">Published</span>
                    )}
                </div>
            </td>
            <td>
                <div className={'flex items-start justify-start !h-[100%]'}>
                    <span className={'!mt-3.5'}>{vod.genre}</span>
                </div>
            </td>
            <td>
                <div className={'flex !mt-2.5  items-start justify-start !h-[100%]'}>
                    {
                        vod.shared_streams.length > 1 ?
                            <Menu.Root>
                                <Menu.Trigger className="!pt-3.5 ">
                                    <span className={'!text-[#E966AE]'}>{vod.shared_streams.length} Channels</span>
                                </Menu.Trigger>
                                <Menu.Portal>
                                    <Menu.Positioner className="outline-none" sideOffset={8}>
                                        <Menu.Popup
                                            className="origin-[var(--transform-origin)] rounded-md bg-[canvas] py-1 text-white shadow-lg shadow-gray-200 outline outline-1 outline-gray-200 transition-[transform,scale,opacity] data-[ending-style]:scale-90 data-[ending-style]:opacity-0 data-[starting-style]:scale-90 data-[starting-style]:opacity-0 dark:shadow-none dark:-outline-offset-1 dark:outline-gray-300">
                                            {
                                                vod.shared_streams && <>
                                                    {
                                                        vod.shared_streams.map((item, index) => (
                                                            <React.Fragment key={"channel-" + index}>
                                                                <Menu.Item
                                                                    className="flex cursor-default py-2 pr-8 pl-4 text-sm leading-4 outline-none select-none data-[highlighted]:relative data-[highlighted]:z-0 data-[highlighted]:text-gray-50 data-[highlighted]:before:absolute data-[highlighted]:before:inset-x-1 data-[highlighted]:before:inset-y-0 data-[highlighted]:before:z-[-1] data-[highlighted]:before:rounded-sm data-[highlighted]:before:bg-[#5B2B45]">
                                                                    {item.name}
                                                                </Menu.Item>
                                                                {
                                                                    index !== vod.shared_streams.length - 1 &&
                                                                    <Menu.Separator
                                                                        className="mx-0 my-1.5 h-px bg-gray-200"/>
                                                                }
                                                            </React.Fragment>
                                                        ))
                                                    }
                                                </>
                                            }
                                        </Menu.Popup>
                                    </Menu.Positioner>
                                </Menu.Portal>
                            </Menu.Root>
                            :
                            <span className={'!text-[#E966AE] !mt-2.5'}>{vod.shared_streams?.[0]?.name}</span>
                    }
                </div>
            </td>
            <td>
                <div className={'flex items-start justify-start !h-[100%]'}>
                    <span className={'!mt-3.5'}>{secondsToHMS(vod.duration)}</span>
                </div>
            </td>
            <td>
                <div className={'flex items-start justify-start !h-[100%]'}>
                    <span className={'!mt-3.5'}>{formatDate(vod.created_at)}</span>
                </div>
            </td>
            <td>
                <div className={'flex items-start justify-start !h-[100%]'}>
                    <div
                        className={`${loading === vod.id ? "flex" : "!hidden"} h-[70%] w-[50%] items-center justify-end`}>
                        <IonSpinner name="dots"></IonSpinner>
                    </div>

                    <IonButton shape="round" color={"transparent"}
                                className={`${loading === null || loading !== vod.id ? "flex" : "!hidden"} !mt-3`}
                                onClick={handleClick}>
                        <IonIcon slot="icon-only" size={"48"} icon={ellipsisVerticalSharp}></IonIcon>
                    </IonButton>

                    <AlertDialog.Root open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                        <AlertDialog.Portal>
                            <AlertDialog.Backdrop
                                className="fixed inset-0 bg-[#5B2B45] opacity-20 transition-all duration-150 data-[ending-style]:opacity-0 data-[starting-style]:opacity-0 dark:opacity-70"/>
                            <AlertDialog.Popup
                                className="fixed top-1/2 left-1/2 -mt-8 w-96 max-w-[calc(100vw-3rem)] -translate-x-1/2 -translate-y-1/2 rounded-lg bg-[#000000] p-6 text-white transition-all duration-150 data-[ending-style]:scale-90 data-[ending-style]:opacity-0 data-[starting-style]:scale-90 data-[starting-style]:opacity-0 dark:outline-gray-300">
                                <AlertDialog.Title className="-mt-1.5 mb-5 text-lg text-[#F5485E] font-black">
                                    Delete VOD?
                                </AlertDialog.Title>
                                <AlertDialog.Description className="mb-6 text-base text-gray-100">
                                    You can’t undo this action.
                                </AlertDialog.Description>
                                <div className="flex justify-end gap-4">
                                    <AlertDialog.Close
                                        className="flex h-10 items-center justify-center rounded-md border border-gray-200 bg-gray-50 px-3.5 text-base font-medium text-gray-900 select-none hover:bg-gray-100 focus-visible:outline focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-blue-800 active:bg-gray-100">
                                        Cancel
                                    </AlertDialog.Close>
                                    <AlertDialog.Close onClick={deleteItem}
                                                        className="flex h-10 items-center justify-center rounded-md border border-[#5B2B45] bg-[#5B2B45] px-3.5 text-base font-medium text-[#fff] select-none hover:bg-gray-100 hover:text-[#5B2B45] focus-visible:outline focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-blue-800 active:bg-gray-100">
                                        Delete
                                    </AlertDialog.Close>
                                </div>
                            </AlertDialog.Popup>
                        </AlertDialog.Portal>
                    </AlertDialog.Root>

                    <IonPopover
                        event={profilePopover.event}
                        isOpen={profilePopover.showPopover}
                        onDidDismiss={() => setProfilePopover({showPopover: false})}
                    >
                        <IonList className={'!w-[200px]'}>
                            <IonItem
                                className={"cursor-pointer"}
                                lines="none"
                                onClick={() => {
                                    setProfilePopover({showPopover: false});
                                    openModal(vod)
                                }}
                            >
                                <svg xmlns='http://www.w3.org/2000/svg' width={"24"} height={"24"}
                                        className='ionicon' viewBox='0 0 512 512'>
                                    <title>Create</title>
                                    <path
                                        d='M384 224v184a40 40 0 01-40 40H104a40 40 0 01-40-40V168a40 40 0 0140-40h167.48'
                                        strokeLinecap='round' strokeLinejoin='round'
                                        fill={'none'} stroke={"#fff"} strokeWidth={"12"}/>
                                    <path
                                        fill={'none'} stroke={"#fff"} strokeWidth={"12"}
                                        d='M459.94 53.25a16.06 16.06 0 00-23.22-.56L424.35 65a8 8 0 000 11.31l11.34 11.32a8 8 0 0011.34 0l12.06-12c6.1-6.09 6.67-16.01.85-22.38zM399.34 90L218.82 270.2a9 9 0 00-2.31 3.93L208.16 299a3.91 3.91 0 004.86 4.86l24.85-8.35a9 9 0 003.93-2.31L422 112.66a9 9 0 000-12.66l-9.95-10a9 9 0 00-12.71 0z'/>
                                </svg>
                                &nbsp;&nbsp;&nbsp;&nbsp;
                                <span className={'text-[#fff]'}>Edit VoD</span>
                            </IonItem>
                            <IonItem
                                className={"cursor-pointer"}
                                lines="none"
                                onClick={handleDelete}
                            >
                                <svg xmlns='http://www.w3.org/2000/svg' width={"24"} height={"24"}
                                        className='ionicon' fill={"#fff"} viewBox='0 0 512 512'>
                                    <title>Trash</title>
                                    <path
                                        d='M112 112l20 320c.95 18.49 14.4 32 32 32h184c17.67 0 30.87-13.51 32-32l20-320'
                                        strokeLinecap='round' strokeLinejoin='round'
                                        fill={'none'} stroke={"#fff"} strokeWidth={"12"}/>
                                    <path strokeLinecap='round' strokeMiterlimit='10' d='M80 112h352'
                                            fill={'none'} stroke={"#fff"} strokeWidth={"12"}/>
                                    <path
                                        d='M192 112V72h0a23.93 23.93 0 0124-24h80a23.93 23.93 0 0124 24h0v40M256 176v224M184 176l8 224M328 176l-8 224'
                                        strokeLinecap='round' strokeLinejoin='round'
                                        fill={'none'} stroke={"#fff"} strokeWidth={"12"}/>
                                </svg>
                                &nbsp;&nbsp;&nbsp;&nbsp;
                                <span className={'text-[#fff]'}>Delete VoD</span>
                            </IonItem>
                        </IonList>
                    </IonPopover>
                </div>
            </td>
        </tr>
    );
};

export const VodGrid: React.FC<VodListProps> = ({
                                                    vod,
                                                    selection,
                                                    loading,
                                                    handleLoading,
                                                    handleCheckboxChange,
                                                    openModal
                                                }) => {

    const dispatch = useDispatch();

    const deleteItem = async (event: any) => {
        handleLoading(vod.id);
        const response = await VodService.deleteVod(vod.id);
        if (response.data.status === "ok") {
            dispatch(deleteVod(vod.id))
            dispatch(setSuccessToast("VOD data deleted"));
        } else {
            dispatch(setErrorToast(response.data.message));
        }
        handleLoading(null);
    };
   
    return (
        <IonCard className="w-full">
            <div className={"flex justify-center h-[150px]"}>
                <img className={` ${vod.logo === null || vod.logo.length === 0 ? "!h-[112.2px] w-auto" : "w-[100%] !h-auto"}`} alt={vod.title}
                     src={vod.logo && vod.logo.length > 0 ? vod.logo : placeholderImage}/>
            </div>
            <IonCardHeader>
                <IonCardTitle className={"!w-[100%] truncate"}>{vod.title}</IonCardTitle>
                <IonCardSubtitle className={'flex items-center !mt-3 justify-between'}>
                    <div className={'!text-[0.8rem]'}>
                        Visibility: {vod.isPrivate || vod.url.trim().length<3? (
                            <span className="!text-[#5C7ECF] !mt-3.5">Private</span>
                        ) : vod.starsAmount && vod.starsAmount > 0 ? (
                            <span className="!text-[#E966AE] !mt-3.5">Paid</span>
                        ) : (
                            <span className="!text-[#31E36C] !mt-3.5">Public</span>
                        )}
                    </div>
                    <div className={'!text-[0.8rem]'}>
                        Status: {vod.url.trim().length<3? (
                            <span className="!mt-3.5">Not published</span>
                        ) : (
                            <span className="!mt-3.5">Published</span>
                        )}
                    </div>
                    <div>
                        {
                            vod.starsAmount && vod.starsAmount > 0 ?
                                <div className="text-[#D4D4D4] !text-[0.8rem]" style={{display: "flex", alignItems: "center"}}>
                                    <span style={{marginRight: "8px"}}>Cost:</span>
                                    <b className={'text-white'}>{vod.starsAmount}</b> &nbsp;
                                    <IonIcon icon={star} size={"md"} style={{color: "#e91e63"}}/>
                                </div> : <></>
                        }
                    </div>
                </IonCardSubtitle>
            </IonCardHeader>

            <IonCardContent className={'text-white flex items-center'}>
                <IonIcon
                    slot="icon-only"
                    icon={gridOutline}
                    color={"dark"}
                />
                &nbsp;&nbsp;&nbsp;
                <b>{vod.genre}</b>
            </IonCardContent>

            <hr className={"!border-t-[#707070]"} style={{borderTopWidth: "1px"}}/>

            <IonButtons className="stream-actions-buttons !justify-around">
                <IonButton onClick={() => {
                    openModal(vod)
                }}>
                    <IonIcon
                        slot="icon-only"
                        icon={createOutline}
                        color={"dark"}
                    />
                </IonButton>
                <AlertDialog.Root>
                    <AlertDialog.Trigger>
                        <IonButton>
                            <IonIcon
                                slot="icon-only"
                                icon={informationCircleOutline}
                                color={"dark"}
                            />
                        </IonButton>
                    </AlertDialog.Trigger>
                    <AlertDialog.Portal>
                        <AlertDialog.Backdrop
                            className="fixed inset-0 bg-[#fff] opacity-20 transition-all duration-150 data-[ending-style]:opacity-0 data-[starting-style]:opacity-0 dark:opacity-70"/>
                        <AlertDialog.Popup
                            className="fixed top-1/2 left-1/2 -mt-8 w-96 max-w-[calc(100vw-3rem)] -translate-x-1/2 -translate-y-1/2 rounded-lg bg-[#1E1E1E] p-6 text-white transition-all duration-150 data-[ending-style]:scale-90 data-[ending-style]:opacity-0 data-[starting-style]:scale-90 data-[starting-style]:opacity-0 dark:outline-gray-300">
                            <AlertDialog.Title className="-mt-1.5 text-center mb-5 text-lg text-[#white] font-black">
                                <div className={'flex items-center justify-center flex-col gap-3'}>
                                    <img
                                        alt={vod.title}
                                        className={`!rounded-md ${vod.logo ? 'w-[100%]' : 'w-[133.33px]'} h-auto`}
                                        src={vod.logo ?? "data:image/png;base64,"}
                                    />
                                    <span className={"mt-1"}>{vod.title}</span>

                                    <p className={"text-base text-gray-300 font-normal"}>
                                        {vod.description}
                                    </p>
                                </div>
                            </AlertDialog.Title>
                            <AlertDialog.Description className="mb-6 text-base text-gray-300">
                                <div
                                    className={'flex item-center border-0 py-3 border-t-[1px] border-[#707070] justify-between'}>
                                    <span className={'text-start'}>Channel</span>
                                    <span className={'text-end font-bold text-[#E966AE]'}>
                                        {
                                            vod.shared_streams.length > 1 ?
                                                <>
                                                    {
                                                        vod.shared_streams && <>
                                                            {
                                                                vod.shared_streams.map((item, index) => (
                                                                    <React.Fragment key={"channel-" + index}>
                                                                        <li
                                                                            className="flex cursor-default py-2 pr-8 pl-4 text-sm leading-4 outline-none select-none data-[highlighted]:relative data-[highlighted]:z-0 data-[highlighted]:text-gray-50 data-[highlighted]:before:absolute data-[highlighted]:before:inset-x-1 data-[highlighted]:before:inset-y-0 data-[highlighted]:before:z-[-1] data-[highlighted]:before:rounded-sm data-[highlighted]:before:bg-[#5B2B45]">
                                                                            {item.name}
                                                                        </li>
                                                                        {
                                                                            index !== vod.shared_streams.length - 1 && <br/>
                                                                        }
                                                                    </React.Fragment>
                                                                ))
                                                            }
                                                        </>
                                                    }
                                                </>
                                                :
                                                <span className={'!text-[#E966AE] !mt-2.5'}>{vod.shared_streams?.[0]?.name}</span>
                                        }
                                    </span>
                                </div>
                                <div
                                    className={'flex item-center border-0 py-3 border-t-[1px] border-[#707070] justify-between'}>
                                    <span className={'text-start'}>Genre</span>
                                    <span className={'text-end font-bold text-white'}>{vod.genre}</span>
                                </div>
                                <div
                                    className={'flex item-center border-0 py-3 border-t-[1px] border-[#707070] justify-between'}>
                                    <span className={'text-start'}>Visibility</span>
                                    <span className={'text-end'}>
                                        {vod.isPrivate || vod.url.trim().length<3? (
                                            <span className="!text-[#5C7ECF] !mt-3.5">Private</span>
                                        ) : vod.starsAmount && vod.starsAmount > 0 ? (
                                            <span className="!text-[#E966AE] !mt-3.5">Paid</span>
                                        ) : (
                                            <span className="!text-[#31E36C] !mt-3.5">Public</span>
                                        )}
                                    </span>
                                </div>
                                <div
                                    className={'flex item-center border-0 py-3 border-t-[1px] border-[#707070] justify-between'}>
                                    <span className={'text-start'}>Status</span>
                                    <span className={'text-end'}>
                                        { vod.url.trim().length<3? (
                                            <span className="!mt-3.5">Not published</span>
                                        ) : (
                                            <span className="!mt-3.5">Published</span>
                                        )}
                                    </span>
                                </div>
                                <div
                                    className={'flex item-center border-0 py-3 border-t-[1px] border-[#707070] justify-between'}>
                                    <span className={'text-start'}>Duration</span>
                                    <span className={'text-end'}>{secondsToHMS(vod.duration)}</span>
                                </div>
                                <div
                                    className={'flex item-center border-0 py-3 border-t-[1px] border-b-[1px] border-[#707070] justify-between'}>
                                    <span className={'text-start'}>Added on</span>
                                    <span className={'text-end'}>{formatDate(vod.created_at)}</span>
                                </div>
                            </AlertDialog.Description>
                            <div className="flex justify-end gap-4">
                                <AlertDialog.Close
                                    className="flex h-10 items-center justify-center rounded-md border border-gray-200 bg-gray-50 px-3.5 text-base font-medium text-gray-900 select-none hover:bg-gray-100 focus-visible:outline focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-blue-800 active:bg-gray-100">
                                    Close
                                </AlertDialog.Close>
                            </div>
                        </AlertDialog.Popup>
                    </AlertDialog.Portal>
                </AlertDialog.Root>
                <AlertDialog.Root>
                    <AlertDialog.Trigger>
                        <IonButton>
                            <IonIcon
                                slot="icon-only"
                                icon={trashOutline}
                                color={"danger"}
                            />
                        </IonButton>
                    </AlertDialog.Trigger>
                    <AlertDialog.Portal>
                        <AlertDialog.Backdrop
                            className="fixed inset-0 bg-[#5B2B45] opacity-20 transition-all duration-150 data-[ending-style]:opacity-0 data-[starting-style]:opacity-0 dark:opacity-70"/>
                        <AlertDialog.Popup
                            className="fixed top-1/2 left-1/2 -mt-8 w-96 max-w-[calc(100vw-3rem)] -translate-x-1/2 -translate-y-1/2 rounded-lg bg-[#000000] p-6 text-white transition-all duration-150 data-[ending-style]:scale-90 data-[ending-style]:opacity-0 data-[starting-style]:scale-90 data-[starting-style]:opacity-0 dark:outline-gray-300">
                            <AlertDialog.Title className="-mt-1.5 mb-5 text-lg text-[#F5485E] font-black">
                                Delete VOD?
                            </AlertDialog.Title>
                            <AlertDialog.Description className="mb-6 text-base text-gray-100">
                                You can’t undo this action.
                            </AlertDialog.Description>
                            <div className="flex justify-end gap-4">
                                <AlertDialog.Close
                                    className="flex h-10 items-center justify-center rounded-md border border-gray-200 bg-gray-50 px-3.5 text-base font-medium text-gray-900 select-none hover:bg-gray-100 focus-visible:outline focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-blue-800 active:bg-gray-100">
                                    Cancel
                                </AlertDialog.Close>
                                <AlertDialog.Close onClick={deleteItem}
                                                   className="flex h-10 items-center justify-center rounded-md border border-[#5B2B45] bg-[#5B2B45] px-3.5 text-base font-medium text-[#fff] select-none hover:bg-gray-100 hover:text-[#5B2B45] focus-visible:outline focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-blue-800 active:bg-gray-100">
                                    Delete
                                </AlertDialog.Close>
                            </div>
                        </AlertDialog.Popup>
                    </AlertDialog.Portal>
                </AlertDialog.Root>
            </IonButtons>
        </IonCard>
    );
};


export const VodChannel: React.FC<VodChannelProps> = ({
                                                    channel,
                                                    openModal
                                                }) => {

    const dispatch = useDispatch();

    return (
        <IonCard className="w-full !bg-transparent cursor-pointer" onClick={() => openModal(channel)}>
            <div className={"flex justify-center h-[150px] relative"}>
                <FallbackImage
                    src={channel.logo ?? placeholderImage}
                    alt={channel.name}
                    placeholder={placeholderImage}
                />
                {
                    channel.shared_vods.length > 0 ?
                        <div
                            style={{
                                background: "transparent linear-gradient(180deg, #AE00B3 0%, #D50087 100%) 0 0 no-repeat padding-box",
                                borderBottomRightRadius: "6px",
                                borderTopRightRadius: "6px"
                            }}
                            className={"absolute right-0 py-2 top-0 !h-[100%] !w-[50px] flex flex-col justify-between items-center"}>
                            <b className={"flex-1 flex justify-center items-center text-[1rem] text-white"}>{channel.shared_vods.length}</b>
                            <IonIcon icon={listOutline} color={"dark"} size={"large"}></IonIcon>
                        </div> : <></>
                }
            </div>
            <IonCardHeader className={"!px-0 !m-0"}>
                <IonCardTitle className={"!w-[100%] flex justify-between items-center !text-[0.85rem]"}>
                    <b className={"!w-[80%] truncate"}>{channel.name}</b>
                    {
                        channel.starsAmount && parseInt(`${channel.starsAmount}`) > 0 ?
                            <div className="text-[#D4D4D4] !text-[0.8rem]" style={{display: "flex", alignItems: "center"}}>
                                <b className={'text-white'}>{channel.starsAmount}</b> &nbsp;
                                <IonIcon icon={star} size={"md"} style={{color: "#e91e63"}}/>
                            </div> : <></>
                    }
                </IonCardTitle>
            </IonCardHeader>
        </IonCard>
    );
};

